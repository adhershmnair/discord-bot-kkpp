...
module.exports = async () => {
  ...
  client.on('guildMemberUpdate', async (oldMember, newMember) => {
    messageListeners['guildMemberUpdate']?.forEach(listener => listener(oldMember, newMember));
  });
  ...
  client.on('interactionCreate', async (interaction) => {
    ...
    switch (interactionType) {
      ...
      case 'MessageComponent':
        if (interaction.customId === 'apply_second_char') {
          // console.log('Second character application button clicked in mountListeners');
        } else if (interaction.customId === 'accept_second_char' || interaction.customId === 'decline_second_char') {
          // console.log('Second character review button clicked in mountListeners');
        } else if (interaction.customId === 'declare-turf') {
          // Let the dedicated turf panel handler handle this
          // It's already handled by the turfPanelInteractionHandler
          // console.log('Turf declaration button clicked in mountListeners - Skipping (handled by dedicated handler)');
          // We don't call any handler here as it's handled separately
        } else if (interaction.customId.startsWith('accept-turf-') || interaction.customId.startsWith('decline-turf-')) {
          // These are handled by the turfResponseHandler, so we don't need to handle them here
          // console.log('Turf response button clicked in mountListeners - Skipping (handled by dedicated handler)');
          // We don't need to handle this here as it's managed by the dedicated handler
        }
        break;
      case 'ApplicationCommandAutocomplete':
        ...
        break;
      case 'ModalSubmit':
        if (interaction.customId === 'second_char_modal') {
          // console.log('Second character application modal submitted in mountListeners');
        } else if (interaction.customId === 'accept_reason_modal' || interaction.customId === 'decline_reason_modal') {
          // console.log('Second character review modal submitted in mountListeners');
        } else if (interaction.customId.startsWith('accept-turf-modal-') || interaction.customId.startsWith('decline-turf-modal-')) {
          // Let the turf response handler handle this
          // console.log('Turf response modal submitted in mountListeners - Skipping (handled by dedicated handler)');
          // This is handled by the turfResponseHandler
        }
        break;
  ...
  // ******** Webhooks ********
  // **************************
  client.on('webhooksUpdate', async (message) => {
